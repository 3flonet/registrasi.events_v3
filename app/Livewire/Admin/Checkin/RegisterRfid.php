...
class RegisterRfid extends Component
{
    ...
    public function findUserByUuid($uuid)
    {
        ...
        if (!$user) {
            $registration = \App\Models\Registration::where('uuid', $uuid)->first();
            if ($registration) {
                if ($registration->user) {
                    $user = $registration->user;
                } else {
                    // Jika ini adalah pendaftaran tamu (Guest), buatkan akun User otomatis
                    // atau hubungkan ke akun yang sudah ada dengan email yang sama.
                    try {
                        // Cek apakah user dengan email tersebut sudah ada di sistem
                        ...
                        // Hubungkan kembali registrasi ke user ini
                        $registration->update(['user_id' => $user->id]);
                    } catch (\Exception $e) {
                        $this->lastScanned = ['status' => 'error', 'message' => 'Gagal mengaitkan profil: ' . $e->getMessage()];
                        return;
                    }
                }
