<x-guest-layout>
    <div class="w-full">
        ...
        <div class="bg-gray-100">
            <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 pt-24 lg:pt-32">
                    ...
                    <div class="col-span-12 lg:col-span-8">
                        ...
                        <div x-data="{ 
                            ...
                            <div x-show="activeSlide === {{ $loop->iteration }}"
                                ...
                                <img src="{{ $event->getFirstMediaUrl('default') }}" alt="{{ $event->name }}" class="w-full h-full object-cover">
                                ...
                                <div class="absolute inset-0 z-10 p-8 sm:p-12 flex flex-col justify-end text-white">
                                    ...
                                    <p class="text-base md:text-lg text-green-100 max-w-3xl mb-5">
                                        <span class="font-semibold">
                                            ...
                                            @else
                                                {{ $event->start_date->locale(app()->getLocale())->translatedFormat('d F Y') }} - {{ $event->end_date->locale(app()->getLocale())->translatedFormat('d F Y') }}
                                            @endif
                                        </span>
                                        ...
                                        <span>
                                            ...
                                            @elseif($event->type === 'hybrid')
                                                {{ $event->venue }} &bull; Online via {{ $event->platform === 'Lainnya...' && isset($event->meeting_info['platform_name']) ? $event->meeting_info['platform_name'] : ($event->platform ?? 'Online') }}
                                            @endif
                                        </span>
                            ...
                            <div class="p-6 text-gray-900 flex-grow">
                                ...
                                <div class="mt-4 text-sm text-gray-500 space-y-2">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span>
                                            @if($event->start_date->isSameDay($event->end_date))
                                                {{ $event->start_date->locale(app()->getLocale())->translatedFormat('d F Y') }}
                                            @elseif($event->start_date->isSameMonth($event->end_date))
                                                {{ $event->start_date->format('d') }} - {{ $event->end_date->locale(app()->getLocale())->translatedFormat('d F Y') }}
                                            @elseif($event->start_date->isSameYear($event->end_date))
                                                {{ $event->start_date->locale(app()->getLocale())->translatedFormat('d F') }} - {{ $event->end_date->locale(app()->getLocale())->translatedFormat('d F Y') }}
                                    ...
                                    <div class="mt-2 space-y-1">
                                        ...
                                        <div class="flex items-start gap-2">
                                            ...
                                            <span class="text-sm text-gray-500">{{ is_array($event->venue) ? ($event->venue['name'] ?? $event->venue) : $event->venue }}</span>
                                        </div>
                                        @endif
