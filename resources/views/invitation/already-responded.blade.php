<x-guest-layout>
    <div class="min-h-screen flex flex-col justify-center items-center pt-6 sm:pt-0 bg-gray-100">
        <div class="w-full sm:max-w-md mt-32 bg-white shadow-md overflow-hidden sm:rounded-lg text-center">
            
            {{-- Event Banner --}}
            @if($event->hasMedia('default'))
                <div class="w-full">
                    <img src="{{ $event->getFirstMediaUrl('default', 'card-banner') }}" 
                         alt="{{ $event->name }}" 
