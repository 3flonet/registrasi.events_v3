<div>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ $user->nama_instansi ?: __('exhibitor.profile_title') }}
        </h2>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div class="md:col-span-1 space-y-6">
                            ...
                            <div class="pt-6 border-t">
                                <h4 class="font-semibold text-gray-800 mb-4">{{ __('exhibitor.social_media_links') }}</h4>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <h1 class="text-3xl font-bold text-greener mt-1">{{ __('exhibitor.booth_no') }}: {{ $user->booth_number }}</h1>
                            ...
                            <div class="mt-6 pt-6 border-t">
                                <h4 class="font-semibold text-gray-800 mb-4">{{ __('exhibitor.contact_info') }}</h4>
                                <div class="space-y-3 text-gray-600">
                                    <p><strong class="w-32 inline-block">{{ __('exhibitor.contact_person') }}</strong>: {{ $user->name }} ({{ $user->jabatan }})</p>
                                    @if($user->phone_instansi)
                                    <p><strong class="w-32 inline-block">{{ __('exhibitor.instansi_phone') }}</strong>: {{ $user->phone_instansi }}</p>
                                    @endif
                                    @if($user->whatsapp)
                                    ...
                                    @if($user->alamat)
                                    <p><strong class="w-32 inline-block">{{ __('exhibitor.instansi_address') }}</strong>: {{ $user->alamat }}</p>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="mt-6 pt-6 border-t">
                                <h4 class="font-semibold text-gray-800 mb-4">{{ __('exhibitor.materials_downloads') }}</h4>
                                <div class="flex flex-wrap gap-4">
                                    ...
                                    <a href="{{ asset('storage/' . $user->document_path) }}" download class="px-5 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 text-sm">
                                        <i class="fas fa-download mr-2"></i> {{ __('exhibitor.download_brochure') }}
                                    </a>
                                    @endif
                                    ...
                                    <a href="{{ $user->document_link }}" target="_blank" class="px-5 py-2 bg-gray-700 text-white font-semibold rounded-lg hover:bg-gray-800 text-sm">
                                        <i class="fas fa-external-link-alt mr-2"></i> {{ __('exhibitor.view_external_material') }}
                                    </a>
                                    @endif
                                </div>
                            </div>
