<div class="{{ $isPicker ? 'p-0 min-h-[400px]' : 'p-10 bg-white shadow-sm rounded-[2rem] min-h-screen border border-gray-100' }} relative"
    ...
    <div x-show="isUploading" 
         ...
         class="fixed bottom-8 right-8 bg-[#1a1235] text-white rounded-2xl shadow-2xl p-6 w-80 z-[9999] border border-indigo-500/30 flex flex-col gap-3 animate-fade-in-up"
         style="display: none;">
        ...
        <div class="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
         ...
         <p class="text-[10px] text-gray-400 text-center">Please do not close this page.</p>
    </div>
    ...
    <div x-show="isDropping" class="absolute inset-0 z-40 bg-indigo-50/90 border-4 border-dashed border-indigo-400 rounded-lg flex items-center justify-center transition-all" style="display: none;">
        <div class="text-center">
            <svg class="w-20 h-20 mx-auto text-indigo-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
             <h3 class="text-2xl font-bold text-indigo-700">Drop files to upload</h3>
        </div>
    </div>
    
    {{-- Disk Usage Indicator --}}
    @if(!$isPicker)
    <div class="mb-8 bg-gray-50 border border-gray-100 rounded-2xl p-6 shadow-sm flex items-center justify-between">
        <div class="flex items-center space-x-3 w-full">
            ...
            <div class="flex-1">
                <div class="flex justify-between text-sm mb-1">
                     <span class="font-medium text-gray-700">Google Drive Storage</span>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 pb-4 border-b border-gray-100 gap-4">
        ...
        <div class="w-full md:w-auto relative flex-shrink-0 md:mx-4">
            ...
            <input type="text" 
                   wire:model.live.debounce.500ms="searchQuery" 
                    ...
                    class="pl-10 w-full md:w-72 text-[10px] uppercase font-medium tracking-widest border-gray-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all placeholder-gray-300 py-4 bg-gray-50 border-none">
        </div>
        ...
        <div class="flex items-center space-x-3 w-full md:w-auto justify-end flex-shrink-0">
            ...
            {{-- VIEW TOGGLE --}}
            <div class="flex bg-gray-100 rounded-2xl p-1 mr-2 border border-gray-200/50">
                <button wire:click="$set('viewMode', 'grid')" 
                        class="{{ $viewMode == 'grid' ? 'bg-[#1a1235] shadow-lg text-white' : 'text-gray-400 hover:text-gray-700' }} p-1.5 rounded-xl transition-all"
                        title="Grid View">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                </button>
                <button wire:click="$set('viewMode', 'list')" 
                        class="{{ $viewMode == 'list' ? 'bg-[#1a1235] shadow-lg text-white' : 'text-gray-400 hover:text-gray-700' }} p-1.5 rounded-xl transition-all"
                        title="List View">
            ...
            @if(empty($searchQuery))
                <div class="relative flex items-center" x-data="{ open: @entangle('isCreatingFolder') }">
                    <template x-if="!open">
                        <button @click="open = true" class="flex items-center px-4 py-3 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border border-gray-100">
                            <i class="fas fa-folder-plus mr-2 text-indigo-500 text-xs"></i> New Directory
                        </button>
                    </template>
                    <template x-if="open">
                        <div class="flex items-center space-x-2 animate-fade-in-right bg-white p-1 rounded-2xl border border-indigo-100 shadow-sm">
                            <input type="text" wire:model.live="newFolderName" placeholder="Directory Name..." 
                                   class="w-32 md:w-48 px-3 py-2 text-[10px] font-black uppercase tracking-widest border-none rounded-xl focus:ring-0 placeholder-gray-300"
                                   @keydown.enter="$wire.createFolder()">
                            <button wire:click="createFolder" class="p-2 text-emerald-500 hover:bg-emerald-50 rounded-xl transition-all"><i class="fas fa-check"></i></button>
                            <button @click="open = false" class="p-2 text-red-500 hover:bg-red-50 rounded-xl transition-all"><i class="fas fa-times"></i></button>
                        </div>
                    </template>
                </div>

                 <label for="gdrive-file-upload" class="flex items-center px-6 py-3 bg-[#1a1235] hover:opacity-90 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest cursor-pointer transition-all shadow-xl shadow-indigo-100 whitespace-nowrap border border-indigo-500">
    ...
    {{-- Filter Tabs --}}
    <div class="flex space-x-1 mb-8 bg-gray-100 p-1 rounded-2xl w-fit border border-gray-200/50">
         <button wire:click="$set('filterType', 'all')" 
                 class="px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all {{ $filterType === 'all' ? 'bg-[#1a1235] text-white shadow-lg' : 'text-gray-400 hover:text-gray-700' }}">
             All Files
         </button>
         <button wire:click="$set('filterType', 'image')" 
                 class="px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all {{ $filterType === 'image' ? 'bg-[#1a1235] text-white shadow-lg' : 'text-gray-400 hover:text-gray-700' }}">
             Images
         </button>
         <button wire:click="$set('filterType', 'document')" 
                 class="px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all {{ $filterType === 'document' ? 'bg-[#1a1235] text-white shadow-lg' : 'text-gray-400 hover:text-gray-700' }}">
             Documents
         </button>
    </div>
    ...
    @if(empty($directories) && count($files) == 0)
        ...
        {{-- Empty State --}}
        <div class="flex flex-col items-center justify-center py-32 text-gray-400 border-2 border-dashed border-gray-100 rounded-3xl bg-gray-50/50">
            <svg class="w-16 h-16 mb-4 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"></path></svg>
             <p class="text-lg">This directory is empty</p>
             <p class="text-sm">Upload a file or create a new directory.</p>
        </div>
        ...
        @if($viewMode === 'grid')
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                ...
                @foreach($directories as $dir)
                    <div class="group relative bg-gray-50 hover:bg-indigo-50 border border-gray-100 hover:border-indigo-200 rounded-2xl p-6 transition-all duration-500 cursor-pointer flex flex-col items-center text-center"
                ...
                @foreach($files as $file)
                    <div class="group relative bg-white hover:shadow-2xl hover:shadow-indigo-100 border border-gray-100 rounded-2xl p-6 transition-all duration-500 flex flex-col justify-between cursor-pointer"
                         {{-- LOGIKA KLIK UTAMA --}}
                         @if($isPicker)
                    ...
                    >
                        ...
                        @if($isPicker)
                            ...
                            <div class="absolute inset-0 bg-indigo-500/10 border-2 border-indigo-500 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                 <span class="bg-[#1a1235] text-white text-[9px] px-4 py-2 rounded-xl shadow-2xl font-black uppercase tracking-widest pointer-events-none">
                                     Select Asset
                                 </span>
                            </div>
                        @else
                            {{-- Tampilan Overlay Normal (Download/Link/Delete) --}}
                            <div class="absolute inset-0 bg-[#1a1235]/80 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-3 backdrop-blur-md">
                            ...
                            >
                                ...
                                <td class="px-6 py-3 text-right space-x-2">
                                    ...
                                    @else
                                        ...
                                        <button wire:click="selectFile('{{ $file['path'] }}')" class="bg-[#1a1235] text-white text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-xl hover:opacity-90 transition-all shadow-lg shadow-indigo-100">
                                            Select
                                        </button>
                                    @endif
                                </td>
    ...
    <script>
        ...
        $wire.on('show-delete-confirmation', () => {
            Swal.fire({
                 title: 'Are you sure?',
                 text: "Deleted files and directories cannot be recovered!",
                 icon: 'warning',
